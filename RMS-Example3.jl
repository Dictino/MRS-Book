...
In the last example we arrived to a pretty good controller that have nice properties

- It is simple (good for implementation an debbuging)
...
- We don't have formal waranties that the vehicle will reach and remain in a recovery area form a set of initial conditions (it usually does, but we are not sure by desgin)

- As the motors are active all the time the control law could drain the batteries fast (bad for a rescue scenario)"""
...
In order to have formal waranties of stability we will define two sets:

$\Omega =\{X_0 | x^2+y^2 <R^2, u=v=r=0\}$

$\Omega_r =\{X_0 | (x-x_r)^2+(y-y_r)^2 < R_f^2\}$

The set $\Omega$ defines the posible initial conditions considered in the optimization problem (a circle of radious $R$) and $\Omega_r$ a neighbourhood of the recovery point that we will **enforce** the trajectory to end (a circle of radious $R_f$).

In this example we will use $R=100m$ and $R_f=3m$

Notice that for the optimization problem to be feasible $R_f$ must be chosen apropiately, in our case we choose $R_f=3m$ because the refecence controller (with parameters $a_1=30$, $a_2=60$ and $\Delta \psi =-63\pi/180$) is able to drive the vehicle to this set.
...
md"""## Energy compsumption

To compute the energy compsumption we integrate the instantaneous power: 

$E=\int_{0}^{T} P(t) \; \mathrm{d}t \approx \Delta t\sum_{i=0}^{T/\Delta t} P(t_i)=J_E$

In order to compute the power we need a model of the porpeller that returns the torque that is needed to apply in any given condition, this model needs also to compute the hydrodimamic drag coefficient for all possible angles of attac $\beta$:  
...
# "Four-Quadrant Model" adapted from de A. Häusler PhD Thesis.
	ly=0.15; #distance from the propeller the symetry axis
	d=0.076; #Helix diametter
	R=d/2; #Helix radii
	rho=1023; #density of water
	
	vap=u-ly.*r; #advance velocity of the porpeller
...
md"Now that we have the torque we can compute the power compsuption using a electric model of the motor:"

# ╔═╡ fa590ceb-e8b4-4e0f-956a-1b0ec9b62d0f
function instant_power_compsumption(ap,u,r)
# adapted from de A. Häusler PhD Thesis. 
	nmax=71.6; #max rps of the motor.
	...
	V=Ra*Ip +Ke*np;
	P_motor=V*Ip;
	P_motor=max(P_motor,0*P_motor); #do not regenerate bawoe by breaking
	P=P_motor+Pp;
	return P
end
...
md"And finally we are ready to compute the energy compsumption"
...
Now the combine both the ISE ant the energy compsumption to get:

$J=(1-\lambda) \frac{J_{ISE}}{J_{ISE_{ref}}} +  \lambda \frac{ J_{E} }{J_{E_{ref}} }$

Where $J_{ISE_{ref}}$ and $J_{E_{ref}}$ are the ISE and energy costs of the reference controler starting from initial condition $x_{0w}=[70.26, 70.26, 5.14, 0, 0, 0]^T$ and $0<\lambda<1$ is a weighting factor that tunes the impotance of ISE and energy in the cost function.

Notice that the normalization has two purposes

- It makes the cost adimensional (we cannot mix jules with $m^2s$)
- It makes the cost easy to interpret: $J=1$ means equal to the reference controller in the wost case for any intitial condition in $\Omega$ (which is attained in $x_{0w}$). Thus the optimal controller must have $J<1$ for all initial conditions in $\Omega$.
...
md"""# Minimax optimization problem
We want the vehicle to:
- Optimize the cost $J$
- Converge to the set $\Omega_r$ form **all** initial condition $x_0 \in \Omega$

thus we perform a minimax optimization


$\boldsymbol{\theta}^* = \underset{\boldsymbol{\theta}}{\operatorname{arg\,min}}\ \underset{\boldsymbol{\chi_0} \in \Omega}{max}\ J_{Tot}(\boldsymbol{\theta},\boldsymbol{\chi_0})$
...
In this this example we will consider $t_f=1280$ and $T_f=1600$.
...
md"""
Again it is importan to choose values that are feasible, in this case these values are fesasible but the reference controller *is not able to satisfy the restriction* as we can see in the figure, at at $t=t_f$ the vehicle is at more that $R_f$ distance of the target point. Thus the optimal controller must improve the reference one.
...
# ╔═╡ c0b5c6e1-2fdb-4865-b6f5-45ae3c577dfd


# ╔═╡ 2e593938-cd95-4eaa-a188-f6f7cbbae531


# ╔═╡ 5a6ad0a6-9402-4b8c-997f-69ebbb2bf73a
md"""## Time to play
We have optimized the cost for several values of $\lambda$ and the best results are:
"""
...
md"""
Now change $\lambda$ to see the tradeoff between converging fast to the recovery point and reducing the energy compsumption:
"""
...
restriction(x_base,y_base) #the reference controller is nor fast enougth

# ╔═╡ 65efcc7e-d127-4a71-9412-3a774b46b507
function select_coef(λ)
	#sellect the appropiate values for this lambda
