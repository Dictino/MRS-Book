...
The code is hidden by defauld, to see the code of the model or the simulation put the cursor on top of the cell and press the eye to show or hide the code
"""
...
3)
    #one liner
	a_p=a_0+Δa*sign(sin(yaw))

4) Play with the offest GO to North
    a_p=a_0+Δa*sign(sin(yaw+Δψ))

5) Play with the offest GO to any direction
    ψ_r=pi/2
...
8) test whith the other motor just change the sign of the relative angle
   a_s=a_0+Δa*sign(sin(-(yaw-ψ_r)+Δψ))
   a_p=0
=#
md"Things to show"
...
# ╔═╡ a1eee006-0c67-409c-b177-fb17333fcc86
@bind Δa Slider(0:0.1:100)

# ╔═╡ dc94e277-946a-47fb-a5bd-18749e8f6b1f
...
function simulator(X0,Tf,dt)
    ...
    for i=1:N 
		#compute the control
        a_s[i],a_p[i]=control(x[i],y[i],yaw[i],u[i],v[i],r[i]);
...
# ╔═╡ d3cc5dba-2d38-4582-9901-c9e1d6dc9fd2
J=dt*sum((x.-set_point[1]).^2+(y.-set_point[2]).^2) / 133346

# ╔═╡ 829f8dc6-c4d0-40db-b5fa-23a4e9d1307d
vy_current

# ╔═╡ 3e1db5e6-1d98-4d5c-ad62-5e3f72df5a35
let
	N=ceil(Tf/dt)
	n=Int(floor(time_slider*(N-1))+1)
	p1=plot(t,u',xlabel="time",ylabel="u",label=:none)
	p1=scatter!(p1,[t[n]],[u[n]],label=:none)
	p2=plot(t,v',xlabel="time",ylabel="v",label=:none)
	p2=scatter!(p2,[t[n]],[v[n]],label=:none)
	p3=plot(t,r',xlabel="time",ylabel="r",label=:none)
	p3=scatter!(p3,[t[n]],[r[n]],label=:none)
	
	l = @layout [a ; b; c]
    plot(p1, p2, p3, layout = l)
end

# ╔═╡ 0e640b67-eaab-4f89-9de3-22944ee0ac7b
let
	N=ceil(Tf/dt)
	n=Int(floor(time_slider*(N-1))+1)
	p1=plot(t,a_s',xlabel="time",ylabel="a_s",label=:none)
	p1=scatter!(p1,[t[n]],[a_s[n]],label=:none)
	p2=plot(t,a_p',xlabel="time",ylabel="a_p",label=:none)
	p2=scatter!(p2,[t[n]],[a_p[n]],label=:none)
	l = @layout [a ; b]
    plot(p1, p2, layout = l)
end
...
# ╔═╡ df935b96-34e9-458e-8e6b-3e3c32c79d21
let
	N=ceil(Tf/dt)
	n=Int(floor(time_slider*(N-1))+1)
    p=plot(y',x',aspect_ratio=:equal,label="Trajectory")
	xlabel!("y") #NED
	ylabel!("x")
	#pinta_base!(p,base)
    p=paint_vehicle!(p,x[n],y[n],yaw[n])
	p=scatter!(p,[set_point[1]],[set_point[2]],label="Setpoint")
	#p2=plot(t,zs,label="Vehicle")
	#p2=plot!(p2,t,t*0.0.+zB,label="Base",xlabel="time",ylabel="z")
	#p2=scatter!(p2,[t[n]],[zs[n]],label=:none)
	#l = @layout [a{0.8*h} ; b]
    #plot(p, p2,layout = l)
end
