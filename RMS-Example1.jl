...
md"Move the time slider to see in which direction the vehicle moves, 
...
function constant_control(x,y,ψ,u,v,r,t)
	...
	a_p=a0	# The control action is some constant you can choose	    
    return (a_s,a_p)
end

# ╔═╡ ce9644c6-8c4d-4266-b12f-860c76cbfbc4
md"To reset the default, just press the play button"
...
md"""# Proposal 2: Asymmetric control
The previous control does not go anywhere. This is because we are applying the same force $a_0$. It does not matter what the orientation of the vehicle is.

In order to change that, and break the symmetry of the circle, we will apply a bigger force ($a_0+\Delta a$) when the vehicle faces East and a smaller one when it points West ($a_0-\Delta a$).


To determine if the vehicle points East or West, we only need to see if $sin(\psi)$ is positive or negative:

Points East: $sin(\psi) > 0 \to a_p=a_0+\Delta a$

points West: $sin(\psi) \le 0 \to a_p=a_0-\Delta a$

Now the vehicle moves in **some** direction, but not necessarily in the direction that we want. You can play with the sliders to change the behaviour.
"""
...
function asymmetric_control(x,y,ψ,u,v,r,t);
    a_s=0.0 # BROKEN ACTUATOR
	if(sin(ψ)>0) # Vehicle points East
		a_p=a0+Δa
	else
		a_p=a0-Δa # Vehicle points West
	end
    return (a_s,a_p)
end

# ╔═╡ f13908d1-3db9-4d08-9e8e-f36a77a39a9e
md"**Hint:** If you want, you can drag and drop the sliders (or any other code cell) to any place. For example, you can move $a_0$ here, and have both $a_0$ and $\Delta a$ in the same place."
...
First, we rewrite the if else statement in a compact form:
...
Note that now the control law switches when $\psi+\Delta \psi= n\pi$ instead of $\psi= n\pi$, so the direction of the average displacement will be rotated an angle $\Delta \psi$.

Now you can **Try to adjust $\Delta \psi$ such that the vehicle goes North** (up in the figure). 

**Hint:** Looking at the previous figure with the default values (press play on the slider to reset the defaults), the direction at which the vehicle goes is close to -60º, which could be a good initial guess.
"""
...
md"""# Proposal 4: Position control
Now that we can go north, we can go in any direction $\psi_r$ by changing the origin of the coordinates $\psi \to \psi - \psi_r$ as follows:


$a_p=a_0 + \Delta a \cdot sign(sin(\psi -\psi_r+\Delta \psi))$

In which direction $\psi_r$ do we want to go? 

Simple, go "*straight to the point*", so we compute the relative orientation of the setpoint with respect to the vehicle:

$ψ_r=atan(y_r-y,x_r-x)$

That's all: this simple two-liner control is able to drive the vehicle close to the recovery point with only one actuator available.
"""
...
Tf3=500 # In this example, we need more time to observe the behaviour.
...
The latter control was developed under the assumption that the starboard actuator fails, but if the portside actuator fails, the situation is analogous. 

In fact, it is just a mirror reflection of the latter, and the only thing that needs to be changed for the controller to work is the sign of the relative angle, i.e. $\psi - \psi_r \to \psi_r - \psi$ as follows:


$ψ_r=atan(y_r-y,x_r-x)$

$a_s=a_0 + \Delta a \cdot sign(sin(\psi_r- \psi +\Delta \psi))$

Now, as you can see, the controller works the same, but the vehicle just turns counterclockwise.
"""
...
function position_control2(x,y,ψ,u,v,r,t)
	...
	a_p=0.0 #NOW THE BROKEN ACTUATOR IS THIS ONE
    return (a_s,a_p)
end
...
manifest_format = "2.0"
project_hash = "683d33bfff68bcdca55c333930e57dcc90c4bf1c"
